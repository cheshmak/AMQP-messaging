...
class Messaging {
  ...
  static serializeJsonObject(object) {
    return zlib.deflateSync(BSON.serialize(object));
  }
  static deserializeBufferToJsonObject(buffer) {
    return BSON.deserialize(zlib.inflateSync(buffer),{
      promoteBuffers: true
    });
  }
  ...
  async addWorker(queueName, workerFunction, options) {
    ...
    channel.consume(queueName, async (msg) => {
      ...
      if (replyTo) {
        await channel.sendToQueue(replyTo, Messaging.serializeJsonObject(response), {
  ...
  async sendPush(queueName, data, options = {}) {
    ...
    await channel.sendToQueue(queueName, Messaging.serializeJsonObject(data), {
  ...
  async publish(queueName, data) {
    ...
    await channel.publish(queueName, '', Messaging.serializeJsonObject(data));
