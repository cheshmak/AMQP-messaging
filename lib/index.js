...
class Messaging {
      ...
      try {
        if (parsed.p) { // p -> isPacked
          for (let i = 0; i < parsed.d.length; i++) {
            try {
              await workerFunction(parsed.d[i]); // d -> data
            } catch (e) {
              logger.log('error', 'worker function rejected in pack', {
                err: e
              });
            }
          }
  ...
  async sendPush(queueName, data, options) {
    if (_.get(options, 'pack.enable') === true && _.isNumber(_.get(options, 'pack.queueSize') && _.isNumber(_.get(options, 'pack.interval')))) {
      return await packQManager.addItemToQueue(queueName, data, {
        queueSize: _.get(options, 'pack.queueSize', 1),
