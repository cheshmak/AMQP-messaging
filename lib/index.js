...
class Messaging {
  ...
  async addWorker(queueName, workerFunction, options) {
    ...
    await assertQueue.workerQueue(channel, queueName);
  ...
  async sendPush(queueName, data, options = {}) {
    ...
    await channel.sendToQueue(queueName, bson.serialize(data), {
      persistent: persistant
    });
    return this;
  }

  async rpcCall(queueName, data) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const channel = await channelManager.getChannel();
        await assertQueue.workerQueue(channel, queueName);
        const replyName = await assertQueue.replyQueue(channel, queueName);
        ...
        const callback = (content) => {
          ...
          let parsed;
          try {
            parsed = bson.deserialize(content, {
              promoteBuffers: true
            });
          } catch (e) {
        ...
        const timeToLive = _.get(queueConfig[queueName], 'timeToLive');
        if (timeToLive > 0) {
  ...
  async publish(queueName, data) {
    ...
    await channel.publish(queueName, '', bson.serialize(data));
