...
class Messaging {
  ...
  async addWorker(queueName, workerFunction, options) {
    ...
    channel.consume(queueName, async (msg) => {
      let parsed;
      try {
        parsed = await Messaging.deserializeBufferToJsonObject(msg.content);
      } catch (e) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const callback = async (content) => {
          ...
          let parsed;
          try {
            parsed = await Messaging.deserializeBufferToJsonObject(content);
          } catch (e) {
