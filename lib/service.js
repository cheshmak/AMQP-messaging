...
service.createNewChannel = function () {
  service._channelWrapper = system.connection.createChannel({
    setup: (channel) => {
      return Q.fcall(() => {
        ...
        const allQueues = _.map(service._queueNames, (name) => {
          return assertQueue.workerQueue(channel, name);
        });
        return Q.all(allQueues);
      });
    }
  });
};

service.addToQueueNames = function (routeName) {
  if (_.indexOf(service._queueNames, routeName) === -1) {
    service._queueNames.push(routeName);
    service.createNewChannel();
  }
};

service.getChannelWrapper = function (renew) {
  if (!service._channelWrapper || renew) {
    service.createNewChannel();
  }
...
service.prototype.addWorker = function (routeName, workerFunction, options) {
  const self = this;
  service.addToQueueNames(routeName);
