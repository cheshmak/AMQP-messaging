...
service.prototype._emptyChannel = function (routeName) {
  const self = this;
  if (_.has(self.channels, routeName) && _.isArray(self.channels[routeName])) {
    self.channels[routeName] = [];
  }
...
service.prototype.cancelWorker = function (routeName) {
  ...
  async.whilst(() => channels.length > 0,(callback) => {
    cancelConsumers(channels, callback);
  }, (err, res) => {
    if (err) {
